...
@interface test_library : SenTestCase <VoltaLibraryObserver>
...
- (void) test_refreshing_the_list_of_subircuits
{
...

  {
    ...
    STAssertTrue([fm copyItemAtPath:sourcePath toPath:targetPath error:&fileOperationError], @"%@", [fileOperationError localizedDescription]);
    [library reloadSubcircuits];
    FXUTAssert([self libraryContainsTestSubcircuit]);

    STAssertTrue([fm removeItemAtPath:targetPath error:&fileOperationError], @"%@", [fileOperationError localizedDescription]);
    [library reloadSubcircuits];
    FXUTAssert(![self libraryContainsTestSubcircuit]);
  }
}
...
- (void) test_thread_safe_access_to_custom_models
{
  ...
  __block int originalNumberOfModelsInSystemGroup = 0;
