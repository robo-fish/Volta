...
struct FXConverter_Node
{
  int32_t nodeNumber;
  FXString nodeName;
  FXStringVector equivalentNames; // Used to map node names in element properties (voltmeter reference nodes) to the final netlist node names.
...
static NSBundle* sLocalizationTableBundle = nil;

static FXString localizedError(FXString errorMessage, ...)
{
  va_list argList;
  va_start(argList, errorMessage);
  NSBundle* localizationTableBundle = (sLocalizationTableBundle == nil) ? [NSBundle mainBundle] : sLocalizationTableBundle;
  FXString result = errorMessage.localize(localizationTableBundle, @"ConversionErrors", argList);
  va_end(argList);
  return result;
}
...
// Electrically equivalent nodes are added to the node's equivalentNodes.
  ...
  if ( !connector.traversed )
  {
    ...
    for( FXString const & connectedElement : connectedElements )
    {
      ...
      if ( model->type == VMT_Node )
      {
        ...
        if ( node->nodeName != connectedElement )
        {
          ...
          {
            node->equivalentNames.push_back(connectedElement);
          }
        }
      }
    }
...
/// Checks if the given node name is in the node table or is an equivalent node of a node in the table.
/// @return a node that is in the node map and is equivalent to the given node.
static FXString findMappedNodeNameForNodeName(FXConverter_NodeAssignmentTable const & nodeTable, FXString const & nodeName)
{
  FXString result;
  for ( FXConverter_NodeAssignmentTable::const_iterator it = nodeTable.begin(); it != nodeTable.end(); it++ )
  {
    FXConverter_NodePtr currentNode = it->second;
    ...
    {
      for ( FXString const & equivalentNodeName : currentNode->equivalentNames )
      {
        if ( equivalentNodeName == nodeName )
        {
          result = currentNode->nodeName;
          break;
        }
      }
    }
...
static void validateNetlistName(VoltaPTElement const & element, VoltaPTModelPtr model, FXConverter_ConverterData & data)
{
  ...
  {
    data.errors.push_back( localizedError("wrong name prefix", element.name.cfString(), prefix.cfString()) );
  }
}
  ...
  FXConverter_ConverterData & data)
{
  ...
  for ( ; orderedPinNames[i] != NULL; i++ )
  {
    ...
    {
      data.errors.push_back( localizedError("pin not wired", orderedPinNames[i], element.name.cfString()) );
    }
  }
}
  ...
  FXString const & elementName )
{
  ...
  if ( result.empty() )
  {
    ...
    {
      errorCollection.push_back(localizedError(errorMessage, elementName.cfString()));
    }
  }
  return result;
}
...
static FXString referenceNodeForMeterElement(VoltaPTElement const & element, FXConverter_ConverterData & data)
{
  FXString referenceNode = PROPERTY_VALUE("ref. node", "", "");
  referenceNode.trimWhitespace();
  FXString result = findMappedNodeNameForNodeName(data.nodeTable, referenceNode);
  if ( !referenceNode.empty() && result.empty() )
  {
    result = referenceNode;
    data.errors.push_back(localizedError("referenced node does not exist", element.name.cfString(), referenceNode.cfString()));
  }
  return result;
}
...
static void processMeterElement_VDC(VoltaPTElement const & element, VoltaPTModelPtr model, FXConverter_ConverterData & data, FXStringVector & outCommands, FXString & outPrint)
{
  ...
  if ( itAnode != data.nodeTable.end() )
  {
    ...
    FXString const referenceNode = referenceNodeForMeterElement(element, data);
...
static void processMeterElement_VAC(VoltaPTElement const & element, VoltaPTModelPtr model, FXConverter_ConverterData & data, FXStringVector & outCommands, FXString & outPrint)
{
  ...
  if ( itAnode != data.nodeTable.end() )
  {
    ...
    {
      data.errors.push_back( localizedError("Unknown scale type.", element.name.cfString()) );
    }
    else
    {
    ...
    case VMT_SW:
      if ( subtype == "sw" )
      {
        ...
        if ( !onOrOff.empty() )
        {
          ...
          {
            data.out << " ?";
            data.errors.push_back( localizedError("Expected ON or OFF", element.name.cfString()) );
          }
          else
          {
            data.out << " " << onOrOff;
    ...
    case VMT_SUBCKT:
      {
        for( VoltaPTPin const & pin : model->pins )
        {
          ...
          if ( it == data.nodeTable.end() )
          {
            ...
            for( VoltaPTMetaDataItem const & item : model->metaData )
            {
              if ( item.first == pin.name )
              {
                ...
                {
                  data.errors.push_back( localizedError("subcircuit pin not wired", pin.name.cfString(), element.name.cfString()) );
                }
                break;
              }
            }
          ...
          else
          {
            ...
            {
              data.errors.push_back( localizedError("subcircuit pin no internal wiring", pin.name.cfString(), element.name.cfString()) );
            }
            data.out << " " << it->second->nodeName;
          }
        }
    ...
    case VMT_XL:
      ...
      else if ( subtype == "tra" )
      {
        ...
        for( VoltaPTProperty const & property : element.properties )
        {
          ...
          {
            data.errors.push_back( localizedError("invalid impedance value", element.name.cfString()) );
          }
        }
      }
      else
...
static void openNetlist(FXConverter_ConverterData & data)
{
  if ( data.subcircuitData->enabled )
  {
    ...
    {
      data.errors.push_back(localizedError("Subcircuit name is missing."));
    }
...
static void checkCircuitIsGrounded(FXConverter_ConverterData & data)
{
  ...
  if ( !data.subcircuitData->enabled )
  {
    ...
    {
      data.errors.push_back(localizedError("The circuit is not grounded."));
    }
  }
}
...
void FXSchematicToNetlistConverter::setConversionErrorMessagesBundle(NSBundle* bundle)
{
  sLocalizationTableBundle = bundle;
}
