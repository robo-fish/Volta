...
@interface test_persistent_types : XCTestCase
@end
...
- (void) test_model_equality
{
  ...
  XCTAssertFalse(model1 == model3, @"The models are equal only if they point to the same object.");
}
...
- (void) test_thread_safety_of_VoltaPTModelPtrVector
{
  ...
  XCTAssertEqual(models.size(), threadCount * numInsertions * repeats / 2, @"Unexpected container size.");
}
